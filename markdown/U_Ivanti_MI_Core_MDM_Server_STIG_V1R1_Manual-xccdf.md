# STIG Benchmark: Ivanti MobileIron Core MDM Server Security Technical Implementation Guide
...
### Rule: The Ivanti MobileIron Core server must limit the number of concurrent sessions per privileged user account to three or less concurrent sessions.

**Rule ID:** `SV-251400r806332_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must initiate a session lock after a 15-minute period of inactivity.

**Rule ID:** `SV-251401r806335_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must enforce the limit of three consecutive invalid logon attempts by a user during a 15-minute time period.

**Rule ID:** `SV-251402r806338_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must display the Standard Mandatory DoD Notice and Consent Banner before granting access to the application.

**Rule ID:** `SV-251403r806341_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must alert the ISSO and SA (at a minimum) in the event of an audit processing failure.

**Rule ID:** `SV-251404r806344_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must back up audit records at least every seven days onto a log management server.

**Rule ID:** `SV-251405r806347_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must be configured to use a DoD Central Directory Service to provide multifactor authentication for network access to privileged and non-privileged accounts.

**Rule ID:** `SV-251406r806350_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must enforce a minimum 15-character password length.

**Rule ID:** `SV-251407r806353_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must prohibit password reuse for a minimum of four generations.

**Rule ID:** `SV-251408r806356_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must enforce password complexity by requiring that at least one uppercase character be used.

**Rule ID:** `SV-251409r806359_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must enforce password complexity by requiring that at least one lowercase character be used.

**Rule ID:** `SV-251410r806362_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must enforce password complexity by requiring that at least one numeric character be used.

**Rule ID:** `SV-251411r806365_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must enforce password complexity by requiring that at least one special character be used.

**Rule ID:** `SV-251412r806368_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must use FIPS-validated SHA-2 or higher hash function to protect the integrity of keyed-hash message authentication code (HMAC), Key Derivation Functions (KDFs), Random Bit Generation, and hash-only applications.

**Rule ID:** `SV-251413r806371_rule`
**Severity:** high
...
### Rule: The Ivanti MobileIron Core server must automatically terminate a user session after an organization-defined period of user inactivity.

**Rule ID:** `SV-251414r806374_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must configure web management tools with FIPS-validated Advanced Encryption Standard (AES) cipher block algorithm to protect the confidentiality of maintenance and diagnostic communications for nonlocal maintenance sessions.

**Rule ID:** `SV-251416r806403_rule`
**Severity:** high
...
### Rule: The Ivanti MobileIron Core server must only allow the use of DoD PKI established certificate authorities for verification of the establishment of protected sessions.

**Rule ID:** `SV-251417r806383_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must be maintained at a supported version.
...
### Rule: The Ivanti MobileIron Core server must use a FIPS-validated cryptographic module to generate cryptographic hashes.

**Rule ID:** `SV-251420r806392_rule`
**Severity:** high
...
### Rule: The Ivanti MobileIron Core server must, at a minimum, off-load audit logs of interconnected systems in real time and off-load standalone systems weekly.

**Rule ID:** `SV-251421r806395_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must be configured in accordance with the security configuration settings based on DoD security configuration or implementation guidance, including STIGs, NSA configuration guides, CTOs, and DTMs.

**Rule ID:** `SV-251422r806398_rule`
**Severity:** medium
...
### Rule: The Ivanti MobileIron Core server must be configured to implement FIPS 140-2 mode for all server and agent encryption.

**Rule ID:** `SV-251423r806401_rule`
**Severity:** high
...
### Rule: The Ivanti MobileIron Core server must configured to lock administrator accounts after three unsuccessful login attempts.
...
### Rule: The Ivanti MobileIron Core server must be configured to lock an administrator's account for at least 15 minutes after the account has been locked because the maximum number of unsuccessful login attempts has been exceeded.

**Rule ID:** `SV-251777r810439_rule`
**Severity:** medium
