# STIG Benchmark: Microsoft Outlook 2016 Security Technical Implementation Guide
...
### Rule: Publishing calendars to Office Online must be prevented.
...
### Rule: Publishing to a Web Distributed and Authoring (DAV) server must be prevented.
...
### Rule: Level of calendar details that a user can publish must be restricted.
...
### Rule: Access restriction settings for published calendars must be configured.
...
### Rule: Outlook Object Model scripts must be disallowed to run for shared folders.
...
### Rule: Outlook Object Model scripts must be disallowed to run for public folders.
...
### Rule: ActiveX One-Off forms must be configured.
...
### Rule: The Add-In Trust Level must be configured.
...
**Description:**
<VulnDiscussion>All installed trusted COM addins can be trusted. Exchange Settings for the addins still override if present and this option is selected.</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: The remember password for internet e-mail accounts must be disabled.
...
**Description:**
<VulnDiscussion>Use this option to hide your user's ability to cache passwords locally in the computer's registry. When configured, this policy will hide the 'Remember Password' checkbox and not allow users to have Outlook remember their password. Note that POP3, IMAP, and HTTP e-mail accounts are all considered Internet e-mail accounts in Outlook. E-mail account options are listed on the Server Type dialog box when users choose 'New' under Tools | Account Settings.</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: Users customizing attachment security settings must be prevented.
...
### Rule: Outlook Security Mode must be configured to use Group Policy settings.
...
### Rule: The ability to display level 1 attachments must be disallowed.
...
### Rule: Level 1 file extensions must be blocked and not removed.
...
### Rule: Level 2 file extensions must be blocked and not removed.
...
### Rule: Scripts in One-Off Outlook forms must be disallowed.
...
### Rule: Custom Outlook Object Model (OOM) action execution prompts must be configured.
...
### Rule: Object Model Prompt for programmatic email send behavior must be configured.
...
### Rule: Object Model Prompt behavior for programmatic address books must be configured.
...
### Rule: Object Model Prompt behavior for programmatic access of user address data must be configured.
...
### Rule: Object Model Prompt behavior for Meeting and Task Responses must be configured.
...
### Rule: Object Model Prompt behavior for the SaveAs method must be configured.
...
### Rule: Object Model Prompt behavior for accessing User Property Formula must be configured.
...
### Rule: Trusted add-ins behavior for email must be configured.
...
### Rule: S/Mime interoperability with external clients for message handling must be configured.
...
### Rule: Message formats must be set to use SMime.
...
### Rule: Run in FIPS compliant mode must be enforced.
...
### Rule: Send all signed messages as clear signed messages must be configured.
...
### Rule: Automatic sending  s/Mime receipt requests must be disallowed.
...
### Rule: Retrieving of CRL data must be set for online action.
...
### Rule: External content and pictures in HTML email must be displayed.
...
### Rule: Permit download of content from safe zones must be configured.
...
### Rule: IE Trusted Zones assumed trusted must be blocked.
...
### Rule: Internet with Safe Zones for Picture Download must be disabled.
...
### Rule: Intranet with Safe Zones for automatic picture downloads must be configured.
...
**Description:**
<VulnDiscussion>This policy setting controls whether pictures and external content in HTML e-mail messages from untrusted senders on the local intranet are downloaded without Outlook users explictly choosing to do so. If you enable this policy setting, Outlook will automatically download external content in all e-mail messages sent over the local intranet and users will not be able to change the setting. If you disable or do not configure this policy setting, Outlook does not consider the local intranet a safe zone, which means that Outlook will not automatically download content from other servers in the Local Intranet zone unless the sender is included in the Safe Senders list. Recipients can choose to download external content from untrusted senders on a message-by-message basis.</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: Always warn on untrusted macros must be enforced.
...
### Rule: Hyperlinks in suspected phishing email messages must be disallowed.
...
### Rule: RPC encryption between Outlook and Exchange server must be enforced.
...
### Rule: Outlook must be configured to force authentication when connecting to an Exchange server.
...
### Rule: Disabling download full text of articles as HTML must be configured.
...
### Rule: Automatic download of Internet Calendar appointment attachments must be disallowed.
...
### Rule: Internet calendar integration in Outlook must be disabled.
...
### Rule: User Entries to Server List must be disallowed.
...
### Rule: Automatically downloading enclosures on RSS must be disallowed.
...
### Rule: Outlook must be configured not to prompt users to choose security settings if default settings fail.
...
**Description:**
<VulnDiscussion>Check to prompt the user to choose security settings if default settings fail; uncheck to automatically select.</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: Outlook minimum encryption key length settings must be set.
...
### Rule: Replies or forwards to signed/encrypted messages must be signed/encrypted.
...
### Rule: Check e-mail addresses against addresses of certificates being used must be disallowed.
...
### Rule: Read signed email as plain text must be enforced.
...
### Rule: The default message format must be set to use Plain Text.

**Rule ID:** `SV-251866r811197_rule`
**Severity:** medium

**Description:**
<VulnDiscussion>Outlook uses HTML as the default email format. HTML format poses a security risk by embedding information into the email itself, which could allow for release of sensitive information. If a user attempted to insert an HTML link into an email message, the link itself may direct to a malicious website. By sending emails in HTML format, the recipient could be subject to becoming infected by the malicious website.</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: Text in Outlook that represents internet and network paths must not be automatically turned into hyperlinks.
