...
**Rule ID:** `SV-228842r1028359_rule`
**Severity:** medium
...
**Rule ID:** `SV-228847r1018772_rule`
**Severity:** medium
...
**Rule ID:** `SV-228851r1018773_rule`
**Severity:** medium
...
**Rule ID:** `SV-228860r1084264_rule`
**Severity:** high
...
### Rule: The Palo Alto Networks security platform must generate an alert to, at a minimum, the ISSO and ISSM when rootkits or other malicious software that allows unauthorized nonprivileged access is detected.

**Rule ID:** `SV-228872r1056123_rule`
**Severity:** medium
...
**Check Text:**
Ask the Administrator how the ISSO and ISSM are receiving alerts (email, SNMP Trap, or Syslog). View the configured Server Profile. If there is no Server Profile for the method explained, this is a finding. View the Log Forwarding Profiles: 1. Go to Objects >> Log Forwarding. 2. Determine which Server Profile is associated with each Log Forwarding Profile. 3. View the Security Policies that are used to filter traffic into the Internal or DMZ zones. 4. Verify the "Profile" column displays the "Antivirus Profile" symbol, the "Vulnerability Protection Profile" symbol, and the "Anti-spyware" symbol (which looks like a magnifying glass on a shield). If the "Profile" column does not display the symbols noted above, this is a finding. Alternatively, verify an Antivirus, an Anti-spyware, and a Vulnerability Protection Profile are included in the actively used profile group that is referenced by the Security policy rule. If the required profiles are not configured, this is a finding.

## Group: SRG-NET-000392-ALG-000148

**Group ID:** `V-228873`

### Rule: The Palo Alto Networks security platform must generate a log record that can be used to send an alert to, at a minimum, the information system security officer (ISSO) and information system security manager (ISSM) when denial-of-service (DoS) incidents are detected.

**Rule ID:** `SV-228873r1028365_rule`
**Severity:** medium
