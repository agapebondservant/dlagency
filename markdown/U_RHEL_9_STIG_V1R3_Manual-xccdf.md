...
### Rule: RHEL 9 must be a vendor-supported release.
...
### Rule: RHEL 9 must implement the Endpoint Security for Linux Threat Prevention tool.
...
### Rule: The graphical display manager must not be the default target on RHEL 9 unless approved.
...
### Rule: RHEL 9 must enable the hardware random number generator entropy gatherer service.
...
### Rule: RHEL 9 systemd-journald service must be enabled.
...
### Rule: The systemd Ctrl-Alt-Delete burst key sequence in RHEL 9 must be disabled.
...
### Rule: The x86 Ctrl-Alt-Delete key sequence must be disabled on RHEL 9.
...
### Rule: RHEL 9 debug-shell systemd service must be disabled.
...
### Rule: RHEL 9 must require a boot loader superuser password.
...
### Rule: RHEL 9 must disable the ability of systemd to spawn an interactive boot process.
...
### Rule: RHEL 9 /boot/grub2/grub.cfg file must be group-owned by root.
...
### Rule: RHEL 9 /boot/grub2/grub.cfg file must be owned by root.
...
### Rule: RHEL 9 must disable virtual system calls.
...
### Rule: RHEL 9 must clear the page allocator to prevent use-after-free attacks.
...
### Rule: RHEL 9 must clear SLUB/SLAB objects to prevent use-after-free attacks.
...
### Rule: RHEL 9 must enable auditing of processes that start prior to the audit daemon.
...
### Rule: RHEL 9 must restrict access to the kernel message buffer.
...
### Rule: RHEL 9 must prevent kernel profiling by nonprivileged users.
...
### Rule: RHEL 9 must prevent the loading of a new kernel for later execution.
...
### Rule: RHEL 9 must restrict exposed kernel pointer addresses access.
...
### Rule: RHEL 9 must enable kernel parameters to enforce discretionary access control on hardlinks.
...
### Rule: RHEL 9 must enable kernel parameters to enforce discretionary access control on symlinks.
...
### Rule: RHEL 9 must disable the kernel.core_pattern.
...
### Rule: RHEL 9 must be configured to disable the Controller Area Network kernel module.

**Rule ID:** `SV-257805r925402_rule`
**Severity:** medium

**Description:**
<VulnDiscussion>Disabling Controller Area Network (CAN) protects the system against exploitation of any flaws in its implementation.</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: RHEL 9 must be configured to disable the FireWire kernel module.
...
### Rule: RHEL 9 must disable the Stream Control Transmission Protocol (SCTP) kernel module.
...
### Rule: RHEL 9 must disable the Transparent Inter Process Communication (TIPC) kernel module.
...
### Rule: RHEL 9 must implement address space layout randomization (ASLR) to protect its memory from unauthorized code execution.
...
### Rule: RHEL 9 must disable access to network bpf system call from nonprivileged processes.
...
### Rule: RHEL 9 must restrict usage of ptrace to descendant processes.
...
### Rule: RHEL 9 must disable storing core dumps.
...
### Rule: RHEL 9 must disable core dumps for all users.
...
### Rule: RHEL 9 must disable acquiring, saving, and processing core dumps.
...
### Rule: RHEL 9 must disable the use of user namespaces.
...
### Rule: The kdump service on RHEL 9 must be disabled.
...
### Rule: RHEL 9 must check the GPG signature of software packages originating from external software repositories before installation.
...
### Rule: RHEL 9 must check the GPG signature of locally installed software packages before installation.
...
### Rule: RHEL 9 must remove all software components after updated versions have been installed.
...
### Rule: RHEL 9 subscription-manager package must be installed.
...
### Rule: RHEL 9 must not have the sendmail package installed.

**Rule ID:** `SV-257827r925468_rule`
**Severity:** medium

**Description:**
<VulnDiscussion>The sendmail software was not developed with security in mind, and its design prevents it from being effectively contained by SELinux. Postfix must be used instead. Satisfies: SRG-OS-000480-GPOS-00227, SRG-OS-000095-GPOS-00049</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: RHEL 9 must not have the nfs-utils package installed.
...
### Rule: RHEL 9 must not have the ypserv package installed.
...
### Rule: RHEL 9 must not have the rsh-server package installed.
...
### Rule: RHEL 9 must not have the telnet-server package installed.
...
### Rule: RHEL 9 must not have the gssproxy package installed.
...
### Rule: RHEL 9 must not have the iprutils package installed.
...
### Rule: RHEL 9 must not have the tuned package installed.
...
### Rule: RHEL 9 must not have the quagga package installed.
...
### Rule: A graphical display manager must not be installed on RHEL 9 unless approved.
...
### Rule: RHEL 9 must have the openssl-pkcs11 package installed.
...
### Rule: RHEL 9 must have the gnutls-utils package installed.
...
### Rule: RHEL 9 must have the nss-tools package installed.
...
### Rule: RHEL 9 must have the rng-tools package installed.
...
### Rule: RHEL 9 must have the s-nail package installed.
...
### Rule: RHEL 9 must use a separate file system for /tmp.
...
### Rule: RHEL 9 must use a separate file system for /var.
...
### Rule: RHEL 9 must use a separate file system for /var/log.
...
### Rule: RHEL 9 must use a separate file system for the system audit data path.

**Rule ID:** `SV-257847r925528_rule`
**Severity:** low

**Description:**
<VulnDiscussion>Placing "/var/log/audit" in its own partition enables better separation between audit files and other system files, and helps ensure that auditing cannot be halted due to the partition running out of space. Satisfies: SRG-OS-000341-GPOS-00132, SRG-OS-000480-GPOS-00227</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: RHEL 9 must use a separate file system for /var/tmp.
...
### Rule: RHEL 9 file system automount function must be disabled unless required.
...
### Rule: RHEL 9 must prevent device files from being interpreted on file systems that contain user home directories.
...
### Rule: RHEL 9 must prevent files with the setuid and setgid bit set from being executed on file systems that contain user home directories.
...
### Rule: RHEL 9 must prevent code from being executed on file systems that contain user home directories.
...
### Rule: RHEL 9 must be configured so that the Network File System (NFS) is configured to use RPCSEC_GSS.
...
### Rule: RHEL 9 must prevent special devices on file systems that are imported via Network File System (NFS).
...
### Rule: RHEL 9  must prevent code from being executed on file systems that are imported via Network File System (NFS).
...
### Rule: RHEL 9 must prevent files with the setuid and setgid bit set from being executed on file systems that are imported via Network File System (NFS).
...
### Rule: RHEL 9 must mount /boot with the nodev option.
...
### Rule: RHEL 9 must prevent files with the setuid and setgid bit set from being executed on the /boot directory.
...
### Rule: RHEL 9 must prevent files with the setuid and setgid bit set from being executed on the /boot/efi directory.
...
### Rule: RHEL 9 must mount /dev/shm with the nodev option.
...
### Rule: RHEL 9 must mount /dev/shm with the nosuid option.
...
### Rule: RHEL 9 must mount /tmp with the nodev option.
...
### Rule: RHEL 9 must mount /tmp with the noexec option.
...
### Rule: RHEL 9 must mount /tmp with the nosuid option.
...
### Rule: RHEL 9 must mount /var with the nodev option.
...
### Rule: RHEL 9 must mount /var/log with the nodev option.
...
### Rule: RHEL 9 must mount /var/log with the noexec option.
...
### Rule: RHEL 9 must mount /var/log with the nosuid option.
...
### Rule: RHEL 9 must mount /var/log/audit with the nodev option.
...
### Rule: RHEL 9 must mount /var/log/audit with the noexec option.
...
### Rule: RHEL 9 must mount /var/log/audit with the nosuid option.
...
### Rule: RHEL 9 must mount /var/tmp with the nodev option.
...
### Rule: RHEL 9 must mount /var/tmp with the noexec option.
...
### Rule: RHEL 9 must mount /var/tmp with the nosuid option.
...
### Rule: RHEL 9 must disable mounting of cramfs.
...
### Rule: RHEL 9 must prevent special devices on non-root local partitions.
...
### Rule: RHEL 9 system commands must have mode 755 or less permissive.
...
### Rule: RHEL 9 library directories must have mode 755 or less permissive.
...
### Rule: RHEL 9 library files must have mode 755 or less permissive.
...
### Rule: RHEL 9 /var/log directory must have mode 0755 or less permissive.
...
### Rule: RHEL 9 audit tools must have a mode of 0755 or less permissive.
...
### Rule: RHEL 9 cron configuration directories must have a mode of 0700 or less permissive.
...
### Rule: All RHEL 9 local initialization files must have mode 0740 or less permissive.
...
### Rule: All RHEL 9 local interactive user home directories must have mode 0750 or less permissive.
...
### Rule: RHEL 9 /etc/group file must have mode 0644 or less permissive to prevent unauthorized access.
...
### Rule: RHEL 9 /etc/gshadow file must have mode 0000 or less permissive to prevent unauthorized access.
...
### Rule: RHEL 9 /etc/passwd file must have mode 0644 or less permissive to prevent unauthorized access.
...
### Rule: RHEL 9 /etc/shadow- file must have mode 0000 or less permissive to prevent unauthorized access.
...
### Rule: RHEL 9 /etc/group file must be owned by root.
...
### Rule: RHEL 9 /etc/group file must be group-owned by root.
...
### Rule: RHEL 9 /etc/gshadow file must be owned by root.
...
### Rule: RHEL 9 /etc/gshadow file must be group-owned by root.
...
### Rule: RHEL 9 /etc/passwd file must be owned by root.
...
### Rule: RHEL 9 /etc/passwd file must be group-owned by root.
...
### Rule: RHEL 9 /etc/shadow file must be owned by root.
...
### Rule: RHEL 9 /etc/shadow file must be group-owned by root.
...
### Rule: RHEL 9 /var/log directory must be owned by root.
...
### Rule: RHEL 9 /var/log directory must be group-owned by root.
...
### Rule: RHEL 9 /var/log/messages file must be owned by root.
...
### Rule: RHEL 9 /var/log/messages file must be group-owned by root.
...
### Rule: RHEL 9 system commands must be owned by root.
...
### Rule: RHEL 9 library files must be owned by root.
...
### Rule: RHEL 9 library files must be group-owned by root or a system account.
...
### Rule: RHEL 9 library directories must be owned by root.
...
### Rule: RHEL 9 audit tools must be owned by root.
...
### Rule: RHEL 9 audit tools must be group-owned by root.
...
### Rule: RHEL 9 cron configuration files directory must be owned by root.
...
### Rule: RHEL 9 cron configuration files directory must be group-owned by root.
...
### Rule: RHEL 9 /etc/crontab file must have mode 0600.
...
### Rule: RHEL 9 /etc/shadow file must have mode 0000 to prevent unauthorized access.
...
### Rule: RHEL 9 must have the firewalld package installed.
...
### Rule: The firewalld service on RHEL 9 must be active.
...
### Rule: RHEL 9 must protect against or limit the effects of denial-of-service (DoS) attacks by ensuring rate-limiting measures on impacted network interfaces are implemented.

**Rule ID:** `SV-257939r925804_rule`
**Severity:** medium

**Description:**
<VulnDiscussion>DoS is a condition when a resource is not available for legitimate users. When this occurs, the organization either cannot accomplish its mission or must operate at degraded capacity. This requirement addresses the configuration of RHEL 9 to mitigate the impact of DoS attacks that have occurred or are ongoing on system availability. For each system, known and potential DoS attacks must be identified and solutions for each type implemented. A variety of technologies exists to limit or, in some cases, eliminate the effects of DoS attacks (e.g., limiting processes or establishing memory partitions). Employing increased capacity and bandwidth, combined with service redundancy, may reduce the susceptibility to some DoS attacks.</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: RHEL 9 network interfaces must not be in promiscuous mode.
...
### Rule: RHEL 9 must enable hardening for the Berkeley Packet Filter just-in-time compiler.
...
### Rule: RHEL 9 must have the chrony package installed.
...
### Rule: RHEL 9 chronyd service must be enabled.
...
### Rule: RHEL 9 must disable the chrony daemon from acting as a server.
...
### Rule: RHEL 9 must disable network management of the chrony daemon.
...
### Rule: RHEL 9 systems using Domain Name Servers (DNS) resolution must have at least two name servers configured.
...
### Rule: RHEL 9 must configure a DNS processing mode set be Network Manager.
...
### Rule: RHEL 9 must be configured to prevent unrestricted mail relaying.
...
### Rule: RHEL 9 must forward mail from postmaster to the root account using a postfix alias.
...
### Rule: RHEL 9 libreswan package must be installed.
...
### Rule: There must be no shosts.equiv files on RHEL 9.
...
### Rule: There must be no .shosts files on RHEL 9.
...
### Rule: RHEL 9 must be configured to use TCP syncookies.

**Rule ID:** `SV-257957r942983_rule`
**Severity:** medium

**Description:**
<VulnDiscussion>Denial of service (DoS) is a condition when a resource is not available for legitimate users. When this occurs, the organization either cannot accomplish its mission or must operate at degraded capacity. Managing excess capacity ensures that sufficient capacity is available to counter flooding attacks. Employing increased capacity and service redundancy may reduce the susceptibility to some DoS attacks. Managing excess capacity may include, for example, establishing selected usage priorities, quotas, or partitioning. Satisfies: SRG-OS-000480-GPOS-00227, SRG-OS-000420-GPOS-00186, SRG-OS-000142-GPOS-00071</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: RHEL 9 must ignore Internet Protocol version 4 (IPv4) Internet Control Message Protocol (ICMP) redirect messages.
...
### Rule: RHEL 9 must not forward Internet Protocol version 4 (IPv4) source-routed packets.
...
### Rule: RHEL 9 must log IPv4 packets with impossible addresses.
...
### Rule: RHEL 9 must log IPv4 packets with impossible addresses by default.
...
### Rule: RHEL 9 must use reverse path filtering on all IPv4 interfaces.
...
### Rule: RHEL 9 must prevent IPv4 Internet Control Message Protocol (ICMP) redirect messages from being accepted.
...
### Rule: RHEL 9 must not forward IPv4 source-routed packets by default.
...
### Rule: RHEL 9 must use a reverse-path filter for IPv4 network traffic when possible by default.
...
### Rule: RHEL 9 must not respond to Internet Control Message Protocol (ICMP) echoes sent to a broadcast address.
...
### Rule: RHEL 9 must limit the number of bogus Internet Control Message Protocol (ICMP) response errors logs.
...
### Rule: RHEL 9 must not send Internet Control Message Protocol (ICMP) redirects.
...
### Rule: RHEL 9 must not allow interfaces to perform Internet Control Message Protocol (ICMP) redirects by default.
...
### Rule: RHEL 9 must not accept router advertisements on all IPv6 interfaces.
...
### Rule: RHEL 9 must ignore IPv6 Internet Control Message Protocol (ICMP) redirect messages.
...
### Rule: RHEL 9 must not forward IPv6 source-routed packets.
...
### Rule: RHEL 9 must not enable IPv6 packet forwarding unless the system is a router.
...
### Rule: RHEL 9 must not accept router advertisements on all IPv6 interfaces by default.
...
### Rule: RHEL 9 must prevent IPv6 Internet Control Message Protocol (ICMP) redirect messages from being accepted.
...
### Rule: RHEL 9 must not forward IPv6 source-routed packets by default.
...
### Rule: All RHEL 9 networked systems must have SSH installed.
...
### Rule: All RHEL 9 networked systems must have and implement SSH to protect the confidentiality and integrity of transmitted and received information, as well as information during preparation for transmission.
...
### Rule: RHEL 9 must have the openssh-clients package installed.
...
### Rule: RHEL 9 must display the Standard Mandatory DOD Notice and Consent Banner before granting local or remote access to the system via a SSH logon.
...
### Rule: RHEL 9 must log SSH connection attempts and failures to the server.
...
### Rule: RHEL 9 SSHD must accept public key authentication.
...
### Rule: RHEL 9 must not permit direct logons to the root account using remote access via SSH.
...
### Rule: RHEL 9 SSH daemon must be configured to use system-wide crypto policies.
...
### Rule: RHEL 9 must implement DOD-approved encryption ciphers to protect the confidentiality of SSH client connections.
...
### Rule: RHEL 9 must implement DOD-approved encryption ciphers to protect the confidentiality of SSH server connections.
...
### Rule: RHEL 9 SSH server must be configured to use only Message Authentication Codes (MACs) employing FIPS 140-3 validated cryptographic hash algorithms.
...
### Rule: RHEL 9 must not allow a noncertificate trusted host SSH logon to the system.
...
### Rule: RHEL 9 must not allow users to override SSH environment variables.
...
### Rule: RHEL 9 must force a frequent session key renegotiation for SSH connections to the server.
...
### Rule: RHEL 9 must be configured so that all network connections associated with SSH traffic terminate after becoming unresponsive.
...
### Rule: RHEL 9 must be configured so that all network connections associated with SSH traffic are terminated after 10 minutes of becoming unresponsive.
...
### Rule: RHEL 9 SSH server configuration file must be group-owned by root.
...
### Rule: RHEL 9 SSH server configuration file must be owned by root.
...
### Rule: RHEL 9 SSH server configuration file must have mode 0600 or less permissive.
...
### Rule: RHEL 9 SSH private host key files must have mode 0640 or less permissive.
...
### Rule: RHEL 9 SSH public host key files must have mode 0644 or less permissive.
...
### Rule: RHEL 9 SSH daemon must not allow compression or must only allow compression after successful authentication.
...
### Rule: RHEL 9 SSH daemon must not allow Kerberos authentication.
...
### Rule: RHEL 9 SSH daemon must not allow rhosts authentication.
...
### Rule: RHEL 9 SSH daemon must not allow known hosts authentication.
...
### Rule: RHEL 9 SSH daemon must disable remote X connections for interactive users.
...
### Rule: RHEL 9 SSH daemon must perform strict mode checking of home directory configuration files.
...
### Rule: RHEL 9 SSH daemon must display the date and time of the last successful account logon upon an SSH logon.
...
### Rule: RHEL 9 SSH daemon must be configured to use privilege separation.
...
### Rule: RHEL 9 SSH daemon must prevent remote hosts from connecting to the proxy display.
...
### Rule: RHEL 9 must prevent a user from overriding the banner-message-enable setting for the graphical user interface.
...
### Rule: RHEL 9 must disable the graphical user interface automount function unless required.
...
### Rule: RHEL 9 must prevent a user from overriding the disabling of the graphical user interface automount function.
...
### Rule: RHEL 9 must prevent a user from overriding the disabling of the graphical user interface autorun function.
...
### Rule: RHEL 9 must prevent a user from overriding the disabling of the graphical user smart card removal action.
...
### Rule: RHEL 9 must prevent a user from overriding the screensaver lock-enabled setting for the graphical user interface.
...
### Rule: RHEL 9 must automatically lock graphical user sessions after 15 minutes of inactivity.
...
### Rule: RHEL 9 must prevent a user from overriding the session idle-delay setting for the graphical user interface.
...
### Rule: RHEL 9 must prevent a user from overriding the session lock-delay setting for the graphical user interface.
...
### Rule: RHEL 9 must conceal, via the session lock, information previously visible on the display with a publicly viewable image.
...
### Rule: RHEL 9 must disable the ability of a user to restart the system from the login screen.
...
### Rule: RHEL 9 must prevent a user from overriding the disable-restart-buttons setting for the graphical user interface.
...
### Rule: RHEL 9 must prevent a user from overriding the Ctrl-Alt-Del sequence settings for the graphical user interface.
...
### Rule: RHEL 9 must disable the user list at logon for graphical user interfaces.
...
### Rule: RHEL 9 must be configured to disable USB mass storage.
...
### Rule: RHEL 9 must have the USBGuard package installed.
...
### Rule: RHEL 9 must have the USBGuard package enabled.
...
### Rule: RHEL 9 must enable Linux audit logging for the USBGuard daemon.
...
### Rule: RHEL 9 Bluetooth must be disabled.
...
### Rule: RHEL 9 user account passwords for new users or password changes must have a 60-day maximum password lifetime restriction in /etc/login.defs.
...
**Description:**
<VulnDiscussion>Any password, no matter how complex, can eventually be cracked; therefore, passwords need to be changed periodically. If RHEL 9 does not limit the lifetime of passwords and force users to change their passwords, there is the risk that RHEL 9 passwords could be compromised.</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: All RHEL 9 local interactive user accounts must be assigned a home directory upon creation.
...
### Rule: RHEL 9 system accounts must not have an interactive login shell.
...
### Rule: All RHEL 9 interactive users must have a primary group that exists.
...
### Rule: RHEL 9 must disable account identifiers (individuals, groups, roles, and devices) after 35 days of inactivity.
...
### Rule: All RHEL 9 local interactive users must have a home directory assigned in the /etc/passwd file.
...
### Rule: RHEL 9 must automatically lock an account when three unsuccessful logon attempts occur.
...
### Rule: RHEL 9 must automatically lock the root account until the root account is released by an administrator when three unsuccessful logon attempts occur during a 15-minute time period.
...
### Rule: RHEL 9 must maintain an account lock until the locked account is released by an administrator.
...
### Rule: RHEL 9 must ensure account lockouts persist.
...
### Rule: RHEL 9 groups must have unique Group ID (GID).
...
### Rule: RHEL 9 must have the tmux package installed.

**Rule ID:** `SV-258063r926176_rule`
**Severity:** medium

**Description:**
<VulnDiscussion>Tmux is a terminal multiplexer that enables a number of terminals to be created, accessed, and controlled from a single screen. Red Hat endorses tmux as the recommended session controlling package. Satisfies: SRG-OS-000030-GPOS-00011, SRG-OS-000028-GPOS-00009</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: RHEL 9 must enable a user session lock until that user re-establishes access using established identification and authentication procedures for command line sessions.
...
### Rule: RHEL 9 must automatically lock command line user sessions after 15 minutes of inactivity.
...
### Rule: RHEL 9 must automatically exit interactive command shell user sessions after 15 minutes of inactivity.
...
### Rule: RHEL 9 must limit the number of concurrent sessions to ten for all accounts and/or account types.
...
### Rule: RHEL 9 must log username information when unsuccessful logon attempts occur.
...
### Rule: RHEL 9 must enforce a delay of at least four seconds between logon prompts following a failed logon attempt.
...
### Rule: RHEL 9 must define default permissions for the bash shell.
...
### Rule: RHEL 9 must define default permissions for the c shell.
...
### Rule: RHEL 9 must define default permissions for all authenticated users in such a way that the user can only read and modify their own files.
...
### Rule: RHEL 9 must define default permissions for the system default profile.
...
### Rule: RHEL 9 must display the date and time of the last successful account logon upon logon.
...
### Rule: RHEL 9 must terminate idle user sessions.
...
### Rule: RHEL 9 must enable the SELinux targeted policy.
...
### Rule: RHEL 9 must have policycoreutils package installed.
...
### Rule: RHEL 9 policycoreutils-python-utils package must be installed.
...
### Rule: RHEL 9 must have the sudo package installed.

**Rule ID:** `SV-258083r926236_rule`
**Severity:** medium

**Description:**
<VulnDiscussion>"sudo" is a program designed to allow a system administrator to give limited root privileges to users and log root activity. The basic philosophy is to give as few privileges as possible but still allow system users to get their work done.</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: RHEL 9 must require reauthentication when using the "sudo" command.
...
### Rule: RHEL 9 must use the invoking user's password for privilege escalation when using "sudo".
...
### Rule: RHEL 9 must require users to reauthenticate for privilege escalation.
...
### Rule: RHEL 9 must restrict privilege elevation to authorized personnel.
...
### Rule: RHEL 9 must restrict the use of the "su" command.

**Rule ID:** `SV-258088r926251_rule`
**Severity:** medium

**Description:**
<VulnDiscussion>The "su" program allows to run commands with a substitute user and group ID. It is commonly used to run commands as the root user. Limiting access to such commands is considered a good security practice. Satisfies: SRG-OS-000373-GPOS-00156, SRG-OS-000312-GPOS-00123</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: RHEL 9 fapolicy module must be installed.
...
### Rule: RHEL 9 fapolicy module must be enabled.
...
### Rule: RHEL 9 must ensure the password complexity module in the system-auth file is configured for three retries or less.
...
### Rule: RHEL 9 must be configured in the password-auth file to prohibit password reuse for a minimum of five generations.
...
### Rule: RHEL 9 must be configured in the system-auth file to prohibit password reuse for a minimum of five generations.
...
### Rule: RHEL 9 must ensure the password complexity module is enabled in the password-auth file.
...
### Rule: RHEL 9 must ensure the password complexity module is enabled in the system-auth file.
...
### Rule: RHEL 9 password-auth must be configured to use a sufficient number of hashing rounds.
...
### Rule: RHEL 9 system-auth must be configured to use a sufficient number of hashing rounds.
...
### Rule: RHEL 9 must enforce password complexity rules for the root account.
...
### Rule: RHEL 9 must enforce password complexity by requiring that at least one numeric character be used.
...
### Rule: RHEL 9 passwords for new users or password changes must have a 24 hours minimum password lifetime restriction in /etc/login.defs.
...
### Rule: RHEL 9 passwords must have a 24 hours minimum password lifetime restriction in /etc/shadow.
...
### Rule: RHEL 9 must require users to provide a password for privilege escalation.
...
### Rule: RHEL 9 passwords for new users must have a minimum of 15 characters.
...
### Rule: RHEL 9 must enforce password complexity by requiring that at least one special character be used.
...
### Rule: RHEL 9 must prevent the use of dictionary words for passwords.
...
### Rule: RHEL 9 must enforce password complexity by requiring that at least one uppercase character be used.
...
### Rule: RHEL 9 must require the change of at least eight characters when passwords are changed.
...
### Rule: RHEL 9 must require the maximum number of repeating characters of the same character class be limited to four when passwords are changed.
...
### Rule: RHEL 9 must require the maximum number of repeating characters be limited to three when passwords are changed.
...
### Rule: RHEL 9 must require the change of at least four character classes when passwords are changed.
...
### Rule: RHEL 9 must be configured so that user and group account administration utilities are configured to store only encrypted representations of passwords.
...
### Rule: RHEL 9 must be configured to use the shadow file to store only encrypted representations of passwords.
...
### Rule: RHEL 9 must not be configured to bypass password requirements for privilege escalation.
...
### Rule: RHEL 9 shadow password suite must be configured to use a sufficient number of hashing rounds.
...
### Rule: RHEL 9 must not have accounts configured with blank or null passwords.
...
### Rule: RHEL 9 must use the CAC smart card driver.
...
### Rule: RHEL 9 must enable certificate based smart card authentication.
...
### Rule: RHEL 9 must implement certificate status checking for multifactor authentication.
...
### Rule: RHEL 9 must have the pcsc-lite package installed.
...
### Rule: The pcscd service on RHEL 9 must be active.
...
### Rule: RHEL 9 must have the opensc package installed.
...
### Rule: RHEL 9 must require authentication to access emergency mode.
...
### Rule: RHEL 9 must require authentication to access single-user mode.
...
### Rule: RHEL 9 must prevent system daemons from using Kerberos for authentication.
...
### Rule: RHEL 9 must prohibit the use of cached authenticators after one day.
...
### Rule: RHEL 9 must use cryptographic mechanisms to protect the integrity of audit tools.
...
### Rule: RHEL 9 must have the rsyslog package installed.
...
### Rule: RHEL 9 must have the packages required for encrypting offloaded audit logs installed.

**Rule ID:** `SV-258141r926410_rule`
**Severity:** medium

**Description:**
<VulnDiscussion>The rsyslog-gnutls package provides Transport Layer Security (TLS) support for the rsyslog daemon, which enables secure remote logging. Satisfies: SRG-OS-000480-GPOS-00227, SRG-OS-000120-GPOS-00061</VulnDiscussion><FalsePositives></FalsePositives><FalseNegatives></FalseNegatives><Documentable>false</Documentable><Mitigations></Mitigations><SeverityOverrideGuidance></SeverityOverrideGuidance><PotentialImpacts></PotentialImpacts><ThirdPartyTools></ThirdPartyTools><MitigationControl></MitigationControl><Responsibility></Responsibility><IAControls></IAControls>
...
### Rule: The rsyslog service on RHEL 9 must be active.
...
### Rule: RHEL 9 must be configured to offload audit records onto a different system from the system being audited via syslog.
...
### Rule: RHEL 9 must authenticate the remote logging server for offloading audit logs via rsyslog.
...
### Rule: RHEL 9 must encrypt the transfer of audit records offloaded onto a different system or media from the system being audited via rsyslog.
...
### Rule: RHEL 9 must be configured to forward audit records via TCP to a different system or media from the system being audited via rsyslog.
...
### Rule: RHEL 9 audit package must be installed.
...
### Rule: RHEL 9 audit service must be enabled.
...
### Rule: RHEL 9 audit system must take appropriate action when an error writing to the audit storage volume occurs.
...
### Rule: RHEL 9 audit system must take appropriate action when the audit storage volume is full.
...
### Rule: RHEL 9 must allocate audit record storage capacity to store at least one week's worth of audit records.
...
### Rule: RHEL 9 must notify the system administrator (SA) and information system security officer (ISSO) (at a minimum) when allocated audit record storage volume 75 percent utilization.
...
### Rule: RHEL 9 must take action when allocated audit record storage volume reaches 95 percent of the audit record storage capacity.
...
### Rule: RHEL 9 must take action when allocated audit record storage volume reaches 95 percent of the repository maximum audit record storage capacity.
...
### Rule: RHEL 9 audit system must take appropriate action when the audit files have reached maximum size.
...
### Rule: RHEL 9 must label all offloaded audit logs before sending them to the central log server.
...
### Rule: RHEL 9 must take appropriate action when the internal event queue is full.
...
### Rule: RHEL 9 System Administrator (SA) and/or information system security officer (ISSO) (at a minimum) must be alerted of an audit processing failure event.
...
### Rule: RHEL 9 audit system must audit local events.
...
### Rule: RHEL 9 audit logs must be group-owned by root or by a restricted logging group to prevent unauthorized read access.
...
### Rule: RHEL 9 audit log directory must be owned by root to prevent unauthorized read access.
...
### Rule: RHEL 9 must periodically flush audit records to disk to prevent the loss of audit records.
...
### Rule: RHEL 9 must produce audit records containing information to establish the identity of any individual or process associated with the event.
...
### Rule: RHEL 9 must write audit records to disk.
...
### Rule: RHEL 9 must allow only the information system security manager (ISSM) (or individuals or roles appointed by the ISSM) to select which auditable events are to be audited.
...
### Rule: RHEL 9 /etc/audit/auditd.conf file must have 0640 or less permissive to prevent unauthorized access.
...
### Rule: RHEL 9 must allocate an audit_backlog_limit of sufficient size to capture processes that start prior to the audit daemon.
...
### Rule: RHEL 9 audispd-plugins package must be installed.
...
### Rule: RHEL 9 must audit uses of the "execve" system call.
...
### Rule: RHEL 9 must audit all uses of the chmod, fchmod, and fchmodat system calls.
...
### Rule: RHEL 9 must audit all uses of the chown, fchown, fchownat, and lchown system calls.
...
### Rule: RHEL 9 must audit all uses of umount system calls.
...
### Rule: RHEL 9 must audit all uses of the chacl command.
...
### Rule: RHEL 9 must audit all uses of the setfacl command.
...
### Rule: RHEL 9 must audit all uses of the chcon command.
...
### Rule: RHEL 9 must audit all uses of the semanage command.
...
### Rule: RHEL 9 must audit all uses of the setfiles command.
...
### Rule: RHEL 9 must audit all uses of the setsebool command.
...
### Rule: RHEL 9 must audit all uses of the rename, unlink, rmdir, renameat, and unlinkat system calls.
...
### Rule: RHEL 9 must audit all uses of the truncate, ftruncate, creat, open, openat, and open_by_handle_at system calls.
...
### Rule: RHEL 9 must audit all uses of the init_module and finit_module system calls.
...
### Rule: RHEL 9 must audit all uses of the chage command.
...
### Rule: RHEL 9 must audit all uses of the chsh command.
...
### Rule: RHEL 9 must audit all uses of the crontab command.
...
### Rule: RHEL 9 must audit all uses of the gpasswd command.
...
### Rule: RHEL 9 must audit all uses of the kmod command.
...
### Rule: RHEL 9 must audit all uses of the newgrp command.
...
### Rule: RHEL 9 must audit all uses of the pam_timestamp_check command.
...
### Rule: RHEL 9 must audit all uses of the passwd command.
...
### Rule: RHEL 9 must audit all uses of the postdrop command.
...
### Rule: RHEL 9 must audit all uses of the postqueue command.
...
### Rule: RHEL 9 must audit all uses of the ssh-agent command.
...
### Rule: RHEL 9 must audit all uses of the ssh-keysign command.
...
### Rule: RHEL 9 must audit all uses of the su command.
...
### Rule: RHEL 9 must audit all uses of the sudo command.
...
### Rule: RHEL 9 must audit all uses of the sudoedit command.
...
### Rule: RHEL 9 must audit all uses of the unix_chkpwd command.
...
### Rule: RHEL 9 must audit all uses of the unix_update command.
...
### Rule: RHEL 9 must audit all uses of the userhelper command.
...
### Rule: RHEL 9 must audit all uses of the usermod command.
...
### Rule: RHEL 9 must audit all uses of the mount command.
...
### Rule: Successful/unsuccessful uses of the init command in RHEL 9 must generate an audit record.
...
### Rule: Successful/unsuccessful uses of the poweroff command in RHEL 9 must generate an audit record.
...
### Rule: Successful/unsuccessful uses of the reboot command in RHEL 9 must generate an audit record.
...
### Rule: Successful/unsuccessful uses of the shutdown command in RHEL 9 must generate an audit record.
...
### Rule: Successful/unsuccessful uses of the umount system call in RHEL 9 must generate an audit record.
...
### Rule: Successful/unsuccessful uses of the umount2 system call in RHEL 9 must generate an audit record.
...
### Rule: RHEL 9 must generate audit records for all account creations, modifications, disabling, and termination events that affect /etc/sudoers.
...
### Rule: RHEL 9 must generate audit records for all account creations, modifications, disabling, and termination events that affect /etc/sudoers.d/ directory.
...
### Rule: RHEL 9 must generate audit records for all account creations, modifications, disabling, and termination events that affect /etc/gshadow.
...
### Rule: RHEL 9 must generate audit records for all account creations, modifications, disabling, and termination events that affect /etc/passwd.
...
### Rule: RHEL 9 must generate audit records for all account creations, modifications, disabling, and termination events that affect /etc/shadow.
...
### Rule: RHEL 9 must generate audit records for all account creations, modifications, disabling, and termination events that affect /var/log/faillock.
...
### Rule: RHEL 9 must generate audit records for all account creations, modifications, disabling, and termination events that affect /var/log/lastlog.
...
### Rule: RHEL 9 must generate audit records for all account creations, modifications, disabling, and termination events that affect /var/log/tallylog.
...
### Rule: RHEL 9 must take appropriate action when a critical audit processing failure occurs.
...
### Rule: RHEL 9 audit system must protect logon UIDs from unauthorized change.
...
### Rule: RHEL 9 audit system must protect auditing rules from unauthorized change.
...
### Rule: RHEL 9 must employ FIPS 140-3 approved cryptographic hashing algorithms for all stored passwords.
...
### Rule: RHEL 9 IP tunnels must use FIPS 140-2/140-3 approved cryptographic algorithms.
...
### Rule: RHEL 9 pam_unix.so module must be configured in the password-auth file to use a FIPS 140-3 approved cryptographic hashing algorithm for system authentication.
...
### Rule: RHEL 9 must have the crypto-policies package installed.
...
### Rule: RHEL 9 crypto policy must not be overridden.
...
### Rule: RHEL 9 must use mechanisms meeting the requirements of applicable federal laws, executive orders, directives, policies, regulations, standards, and guidance for authentication to a cryptographic module.
...
### Rule: RHEL 9 must implement DOD-approved TLS encryption in the GnuTLS package.
...
### Rule: RHEL 9 must implement DOD-approved encryption in the OpenSSL package.
...
### Rule: RHEL 9 must implement DOD-approved TLS encryption in the OpenSSL package.
...
### Rule: RHEL 9 must implement DOD-approved encryption in the bind package.
